...
SDL_Texture* load_texture_from_image(char  *  file_image_name, SDL_Window *window, SDL_Renderer *renderer ){
 ...
 void play_with_texture_5(SDL_Texture *bg_texture,
  ...
  int offset_x = source.w / 4,                // La largeur d'une vignette de l'image
      offset_y = source.h / 5;                // La hauteur d'une vi  SDL_RenderClear(renderer); les vignettes dans le bon ordre pour l'animation
  SDL_Rect state[18];  
...
int main(int argc, char **argv) {
  ...
  SDL_Texture* my_texture;
  my_texture = load_texture_from_image("shadow-80x70.png",window,renderer);
  if (my_texture == NULL) end_sdl(0, "Echec du chargement de l'image dans la texture", window, renderer);  

  SDL_Texture* bg_texture;
  bg_texture = load_texture_from_image("nasa-xFO2Xt33xgI-unsplash.jpg",window,renderer);
